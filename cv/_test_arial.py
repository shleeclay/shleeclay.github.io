...
_set_run(p.add_run("Term project: original thematic maps from public open data."), 9, color=GREY)
...
# (Technical Skills and References sections intentionally omitted)

# ---------------------------------------------------------------- date / signature
p = doc.add_paragraph(); add_right_tab(p); p.paragraph_format.space_before = Pt(18)
_set_run(p.add_run("Seunghyeon Lee"), 9.5, bold=True)
_set_run(p.add_run("\tSignature: ____________________     Date: " + _dtdate.today().strftime("%d %B %Y")),
         9, color=GREY)
