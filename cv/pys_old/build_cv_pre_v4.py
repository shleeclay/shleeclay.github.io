...
contact = [
    ...
    ("shleeclay.github.io", "https://shleeclay.github.io"),
    ("github.com/shleeclay", "https://github.com/shleeclay"),
...
_set_run(p.add_run(" (뉴노멀시티). Urban Issue. ISBN 9791197343100."), 9)
...
projects = [
    ("Janghang National Wetland Restoration — ecosystem monitoring",
     ...
     ["UAV multispectral and LiDAR mapping",
      "Carbon-stock estimation of the site from UAV LiDAR and prediction from the wetland-park vegetation master plan"]),
    ("Carbon storage/sink analysis and inventory for Gyeonggi-do by biotope type",
     ...
     "Prof. Youngkeun Song (SNU)", None,
     ["Analyzed vertical and horizontal forest structure across Gyeonggi Province using ALS",
      "Estimated province-wide forest carbon storage from ALS, airborne hyperspectral, and PlanetScope imagery"]),
    ("Integrated assessment tool for carbon reduction by offset and synergy with ecosystem services",
     ...
     "Prof. Youngkeun Song (SNU)", "1,579",
     ["Acquired 3D structure of abandoned-paddy wetlands using drone LiDAR",
      "Analyzed carbon storage of abandoned paddies and adjacent vegetation"]),
    ("Gyeonggi-do biotope property-information analysis",
     "Gyeonggi Research Institute", "2022.12 — 2023.11", "Researcher",
     "Prof. Youngkeun Song (SNU)", None,
     ["Developed and quantified a method to analyze vegetation layer structure by forest type in Gyeonggi Province"]),
    ("Creation, restoration & management technology of carbon-accumulated abandoned-paddy wetland",
     ...
     "Prof. Bonhak Koo (Sangmyung Univ.)", "547",
    ...
    ("Gwacheon-si urban ecological status mapping",
     ...
     ["Classified forest types and tree species of Gwacheon-si forests and urban green spaces",
      "Analyzed vertical and horizontal forest structure of Gwacheon-si"]),
    ("Real-time web-based positioning surveillance system for introduced exotic species",
     ...
     "Prof. Youngkeun Song (SNU)", "1,208",
     ["Managing researcher — overall scheduling, budget, and research-output management",
      "Attached GPS trackers to four invasive species (red-eared slider, Formosan sika deer, nutria, raccoon) and accumulated movement data",
    ...
    ("IT·ET·BT convergence-based distribution and spread models for introduced exotic species",
     ...
     "Prof. Wanmo Kang (Kookmin Univ.)", "650",
    ...
    ("Redundancy-based green-infrastructure technology for urban ecosystem challenges",
     ...
     "Prof. Youngkeun Song (SNU)", "927",
     ["Installed camera traps to track wild-boar distribution / movement and analyzed home range",
    ...
    ("Monitoring and simulation of climate conditions by spatial type",
     ...
     "Prof. Youngkeun Song (SNU)", None,
     ["Acquired and analyzed field measurements of particulate matter and temperature/humidity by urban spatial type in Seoul",
    ...
    ("Customized habitat-management technique for urban species",
     ...
     "Prof. Chan Park (Univ. of Seoul)", "735",
    ...
    ("Assessment scheme and ecosystem-restoration model by degradation type",
     ...
     "Prof. Youngkeun Song (SNU)", "360",
     ["Performed land-cover classification using satellite imagery"]),
]
for title, funder, period, role, pi, budget, bullets in projects:
    sub = f"{funder} · {role} · PI: {pi}"
    if budget:
        sub += f" · ₩{budget}M (total)"
    entry(title, right=period, sub=sub, bullets=bullets)
...
prof = [
    ("Founder", " — TREE:ID (startup)", "2023.09 — 2025.03",
     "Founded and ran a SaaS startup for urban tree inventory (databasing), diagnosis, and management, "
     "Republic of Korea"),
